...
## 25. 설정 페이지 "내 말투 다시 분석" 기능 추가 (2026-06-15)

| 작업 | 상태 | 대상 파일 |
|------|------|-----------|
| "AI 개인화" 섹션 + "내 인스타 말투 다시 분석" 버튼 추가 | ✅ 완료 | `app/dashboard/settings/page.tsx` |
| `POST /onboarding/{shopId}/reanalyze` 호출 핸들러 (`handleReanalyze`, 연타 방지/로딩/성공·실패 메시지) | ✅ 완료 | `app/dashboard/settings/page.tsx` |
| reanalyze 관련 i18n 키 추가 (KO/EN) | ✅ 완료 | `locales/translations.ts` |

---

## 24. photo_range_max 표시 시점 클램프 (2026-06-14)

| 작업 | 상태 | 대상 파일 |
|------|------|-----------|
| `fetchExisting` 불러올 때 `photo_range_max`를 최대 10으로 클램프 (`Math.min(10, Number(...) \|\| 5)`) | ✅ 완료 | `app/dashboard/auto-upload/page.tsx` |
| `fetchSettings` 불러올 때 `photo_range_max`를 최대 10으로 클램프 | ✅ 완료 | `app/dashboard/settings/page.tsx` |

기존에 20으로 저장된 shop도 불러올 때 10으로 정상 표시. 저장 시점이 아닌 표시 시점 클램프라 기존 DB값은 변경하지 않음 (사장님이 저장 시 10으로 저장됨).
...
## 32. "(주)" 표기 제거 — 개인사업자 기준 정정 (2026-08-03)
