...
// AI 인사이트 카드에 표시할 정적 가이드 항목 (샵별 조작 데이터가 아닌 일반 팁).
const INSIGHT_ITEMS = [
...
function KpiCard({ label, value, change, goodText, growingText, hasData = true }: { label: string; value: string; change: number; goodText: string; growingText: string; hasData?: boolean }) {
  ...
  return (
    <div className="bg-white border border-[#f0e8e8] rounded-[16px] p-6 flex flex-col gap-2">
      ...
      {/* 실데이터가 있을 때만 상태 배지 표시 (데이터 없으면 조작 인상 방지) */}
      {hasData && (
        <span
...
export default function AnalyticsPage() {
  ...
  const m = metricsData;
  const hasMetrics = !!m;
  // 실데이터 없으면 조작 수치(MOCK) 대신 '-' 표시.
  const kpiTotalPosts = m ? String(m.total_posts) : '-';
  const kpiQualityScore = m ? (m.avg_caption_score * 100).toFixed(1) : '-';
  const kpiRetryRate = m ? m.retry_rate : '-';
  const kpiModel = m ? (m.model_distribution.mini > m.model_distribution.full ? 'MINI' : 'FULL') : '-';
  const kpiChange = m ? m.avg_caption_score : 0;

  // 좋아요/댓글 시계열·인기 게시물은 백엔드가 아직 제공하지 않음 → 조작 데이터 대신 빈 배열.
  // 백엔드가 engagement 데이터를 주기 시작하면 여기에 매핑하면 그대로 렌더된다.
  const chartData: { date: string; likes: number; comments: number }[] = [];
  const popularPosts: { id: string; thumbnail: string; likes: number; comments: number; badge: string }[] = [];
  const insights = INSIGHT_ITEMS;
    ...
    <div className="flex flex-row h-screen w-full bg-[#F5F0EB] overflow-hidden">
      ...
      <div className="flex-1 p-10 flex flex-col min-w-0 h-full overflow-y-auto">
        ...
        <div className="grid grid-cols-4 gap-5 mb-10">
          <KpiCard label={t.analytics_page.totalLikes} value={kpiTotalPosts} change={kpiChange >= 0.8 ? 1 : -1} goodText={t.analytics_page.good} growingText={t.analytics_page.growing} hasData={hasMetrics} />
          <KpiCard label={t.analytics_page.totalComments} value={kpiQualityScore} change={kpiChange >= 0.8 ? 1 : -1} goodText={t.analytics_page.good} growingText={t.analytics_page.growing} hasData={hasMetrics} />
          <KpiCard label={t.analytics_page.reach} value={kpiRetryRate} change={kpiChange >= 0.8 ? 1 : -1} goodText={t.analytics_page.good} growingText={t.analytics_page.growing} hasData={hasMetrics} />
          <KpiCard label={t.analytics_page.popularPosts} value={kpiModel} change={kpiChange >= 0.8 ? 1 : -1} goodText={t.analytics_page.good} growingText={t.analytics_page.growing} hasData={hasMetrics} />
        </div>
        ...
        <div className="bg-white border border-[#f0e8e8] rounded-[16px] p-8 mb-10">
          ...
          {chartData.length > 0 ? (
            ...
            <div className="h-[280px] flex items-center justify-center text-[#b0a0a0] text-[0.9rem]" style={{ fontFamily: "'NanumSquare Neo', 'NanumSquare', sans-serif" }}>
              {t.analytics_page.chartEmpty}
            </div>
          )}
        </div>
        ...
        <div className="mb-10">
          ...
          {popularPosts.length > 0 ? (
            ...
            <div className="bg-white border border-[#f0e8e8] rounded-[16px] p-10 flex items-center justify-center text-[#b0a0a0] text-[0.9rem]" style={{ fontFamily: "'NanumSquare Neo', 'NanumSquare', sans-serif" }}>
              {t.analytics_page.popularEmpty}
            </div>
          )}
        </div>
